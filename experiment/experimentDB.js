...
function createtables(){
    exec("create table if not exists personal (personalkey INTEGER PRIMARY KEY,firstname TEXT, lastname TEXT, age INTEGER, gender TEXT, musician TEXT, instrument TEXT, training INTEGER, timeEnter DATE)");
    ...
    exec("create table if not exists ratings (ratingskey integer primary key, sound text, scale integer, rating integer,reactiontime integer, personal_id integer)");
}
...
function addrating()
{
    exec("select max(personalkey) from personal");
    var max=result.value(0,0);
    exec("insert into ratings ('sound', 'scale','rating', 'reactiontime','personal_id') values ('"+arguments[0]+"','"+arguments[1]+"','"+arguments[2]+"','"+arguments[3]+"','"+max+"')");
}

function showratings(){
    exec("select * from ratings");
}
...
function deletepersonal(){     exec("delete from personal");
}

function addpersonal()
{
    exec("insert into personal ('firstname', 'lastname', 'age', 'gender', 'musician', 'instrument', 'training', 'timeEnter') values ('"+arguments[0]+"','"+arguments[1]+"','"+arguments[2]+"','"+arguments[3]+"','"+arguments[4]+"','"+arguments[5]+"','"+arguments[6]+"',DATETIME('NOW'))" );
    post("added personal");
}

function showpersonal(){
    exec("select * from personal");
}
